...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.dwlsk = DWLSK(hidden_features)
        self.act = act_layer()
...
class MultiDetailFusionBridge(nn.Module):
    def __init__(self, in_channels, out_channels=64):
        ...
        self.e_post = nn.Conv2d(out_channels * 4, out_channels, kernel_size=1)
    ...
    def forward(self, e1, e2, e3, e4):
        ...
        # concat
        # fused = torch.concat((pre_e1, pre_e2, pre_e3, pre_e4), dim=1)
        # fused = self.e_post(fused)

        # add
        # fused = pre_e1 + pre_e2 + pre_e3 + pre_e4
...
class LDFormer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            x, ah = self.decoder(res1, res2, res3, res4, h, w, self.num_classes)
            return x, ah
        else:
...
if __name__ == "__main__":
    ...
    net = LDFormer(num_classes=2, decode_channels=256, num_heads=16)
    # print(net)
    out = net(x)
