...
def crop(img,mask,split_size,stride,img_name,save_path,mode):
    ...
    for y in range(0, img.shape[0], stride):
        for x in range(0, img.shape[1], stride):
            ...
            cv2.imwrite(save_path+"img/"+cur_name, img_tile_cut)
            cv2.imwrite(save_path+"mask/"+cur_name, mask_tile_cut)
            index+=1
...
if __name__ == "__main__":
    ...
    path = "/LDFormer/fig_results/massbuilding/"

    save_path = "/LDFormer/fig_results/massbuilding/output/"
